...
**Agenda do 1º refit:** revalidar os β; reavaliar a quarentena de `regiao_ddd`, `idade` e `ocupacao`; threshold de uniformidade de exposição (2,0); simplificações de binning já medidas (`renda_declarada` 5→3 níveis, `status_cadastro` 4→3, `tempo_conhece` 4→3); decidir sobre `religiao` (IV 0,015) e revisitar a D14; re-medir os múltiplos D28/D31 nas 5 faixas; avaliar **trava de maturidade por regime** (D51 — a janela curta preserva o ordenamento dentro do NM, mas inverte no `status_cadastro`).
...
| D51 | 31/jul | **Trava de maturidade por regime — medida, não decidida.** Pergunta do André: dá para ler o sinal com janela de 15–30 dias em vez de esperar 240? Medição em campanhas maduras: **depende do regime**. Para o `status_cadastro`, janela curta **inverte a ordem** — em D+30 o `ex_membro` aparece melhor que o `membro_ativo` (WOE 1,095 vs 0,598), o oposto da maturidade (1,045 vs 1,937), porque ex-membro reativa rápido e membro ativo compra no evento seguinte. Para atributo medido **dentro do não-membro** (testado no `historico_cadastro`, campanhas maduras), a ordem **se preserva** desde D+30 (recadastro_quente 0,365 → 0,392 final; frio 0,089 → 0,165; neutro −0,040 → −0,068) e a taxa de realização é uniforme entre níveis (49–54% até D+30) — é essa uniformidade que faz funcionar, não o tamanho da janela. Implicação: trava em **dois níveis** (universais mantêm 240d; pesquisa poderia usar ~90d restrito a conversões NM) destravaria os atributos de pesquisa do prior ~14 meses antes de mar/2027. Ressalvas: em janela curta as magnitudes ficam comprimidas (`frio` 46% abaixo do final) e testei **um** atributo. **Não implementado** — mexe no coração do desenho e exige rodada de validação própria; era o motivo de não entrar na MR !2426. | pendente |
