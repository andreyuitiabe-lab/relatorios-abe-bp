...
PERIODO_FDS       = "2026-06-27"
PERIODO_FDS_FIM   = "2026-06-29"
PERIODO_FDS_LABEL = "Fds 27–29 jun"
...
# ─── queries (parametrizadas por período) ────────────────────────────────────

def make_queries(inicio: str, fim: str) -> dict:
    base_filter = f"""
      SPLIT(nm_hotlead_type, ' - ')[SAFE_OFFSET(0)] IN {TIPOS_PRINCIPAIS}
      AND nm_hotlead_type IS NOT NULL
      AND DATE(dt_created_at) BETWEEN '{inicio}' AND '{fim}'
      AND (
...
SELECT
  d.canal, d.origem, d.tipo_hotlead, d.produto,
...
def build_period(inicio: str, fim: str, label: str, min_leads: int = 20) -> dict:
    q = make_queries(inicio, fim)

    print(f"  [{label}] resumo por canal...", flush=True)
    resumo = {}
    for r in bq(q["resumo"]):
        resumo[r["canal"]] = {
            "leads":       ii(r["qt_leads"]),
    ...
    print(f"  [{label}] por tipo+origem...", flush=True)
    tipo_idx = {}
    for r in bq(q["por_tipo"]):
    ...
    print(f"  [{label}] matriz IA vs Comercial...", flush=True)
    mat_idx = defaultdict(dict)
    for r in bq(q["matriz"], max_rows=10000):
    ...
    print(f"  [{label}] upsell pós-venda (7 dias)...", flush=True)
    upsell_idx = {}
    for r in bq(q["upsell"], max_rows=10000):
    ...
    for (origem, tipo, produto), canais in sorted(mat_idx.items()):
        ...
        if ia_leads < min_leads and com_leads < min_leads:
            continue
        por_matriz.append({
            "origem":     origem,
            "tipo":       tipo,
...
def build() -> dict:
    data_jun = build_period(PERIODO,     PERIODO_FIM,     PERIODO_LABEL,    min_leads=20)
    data_fds = build_period(PERIODO_FDS, PERIODO_FDS_FIM, PERIODO_FDS_LABEL, min_leads=5)
...
if __name__ == "__main__":
    ...
    print(f"Refreshing IA vs Comercial ({PERIODO_LABEL} + {PERIODO_FDS_LABEL})...")
    try:
        data = build()
