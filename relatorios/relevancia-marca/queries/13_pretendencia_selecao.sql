-- TESTE DE SELEÇÃO (pre-trend / diff-in-diff): quem assiste sabatina já comprava mais ANTES?
-- Se a taxa em D-15..D-1 já for tão alta quanto em D+1..D+14, o "lift" é seleção, não efeito.
-- Mesmo desenho do v3, medindo os dois lados da janela para o mesmo pessoa-dia.
WITH sessoes AS (
...
compras AS (
  ...
  WHERE t.nm_status = 'approved' AND t.bl_is_renovation = FALSE
    AND DATE(t.dt_ordered_at) BETWEEN '2026-02-01' AND '2026-08-21'
),
...
base AS (
  SELECT pd.email, pd.dia, pd.eh_freemium,
    ...
    (SELECT COUNT(*) FROM compras c WHERE c.email = pd.email
       AND c.dia_compra BETWEEN DATE_SUB(pd.dia, INTERVAL 15 DAY) AND DATE_SUB(pd.dia, INTERVAL 1 DAY)) AS pre
  FROM pessoa_dia pd
...
SELECT
  ...
  ROUND(100 * COUNTIF(pre > 0) / COUNT(*), 3) AS tx_pre_pct,
  ROUND(100 * COUNTIF(pos > 0) / COUNT(*), 3) AS tx_pos_pct,
  ROUND(100 * COUNTIF(pos > 0) / COUNT(*) - 100 * COUNTIF(pre > 0) / COUNT(*), 3) AS delta_pp
FROM base
GROUP BY 1, 2, 3
ORDER BY 1, 2, 3
